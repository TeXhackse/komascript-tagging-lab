% ======================================================================
% scrlttr2-de.tex
% Copyright (c) Markus Kohm, 2002-2026
%
...
\KOMAProvidesFile{scrlttr2-de.tex}%
                 [$Date: 2026-08-28 14:44:33 +0200 (Fri, 28 Aug 2026) $
...
\else%
  ...
  \LaTeX{} die Möglichkeit, während des \LaTeX-Laufs eines Dokuments
  zusätzliche \PName{Anweisungen} ausführen zu lassen. Zu diesem Zweck stellt
  der \LaTeX-Kern die Anweisungen \Macro{AtEndOfClass}\IndexCmd{AtEndOfClass}
  und \Macro{AtBeginDocument}\IndexCmd{AtBeginDocument} zur Verfügung. Man
  nennt solche Eingriffspunkte auch \emph{hooks}\Index{hook=\emph{hook}}, also
  Haken. \KOMAScript{} fügt zwei weitere Haken hinzu, die mit
  \Macro{AtBeginLetter} und
  \Macro{AtEndLetter}\ChangedAt{v2.95}{\Class{scrlttr2}} mit Inhalt versehen
  werden können. Wie man schon daran erkennt, dass die \LaTeX-Kern-Anweisungen
  für Haken nicht in \cite{latex:usrguide-historic} sondern in
  \cite{latex:clsguide} dokumentiert sind, sind diese Anweisungen eigentlich
  eher für Paket- und Klassenautoren gedacht. Bei Briefen kann es jedoch
  sinnvolle Anwendungen für die beiden neuen Haken auch auf Benutzerebene
  geben.\Index[indexmain]{Haken}%
  %
  \begin{Example}
...
\Macro{parbox}-Anweisung (siehe \cite{latex:usrguide-historic}) mehrere Boxen
...
sollte man dabei auch Zugriff auf andere Variablen mit Hilfe von
...
\begin{Declaration}
  \OptionVName{shrinkfirstpage}{Ein-Aus-Wert}
...
\end{Explain}
Soll\ChangedAt{v3.48}{\Class{scrlttr2}\and \Package{scrletter}} der
Textbereich nicht automatisch verkleiner werden, um Kollisionen von Text und
Fuß zu verhindern, so kann dies mit \OptionValue{shrinkfirstpage}{false}%
\important{\OptionValue{shrinkfirstpage}{false}} erreicht werden. Soll der
Textbereich auch automatisch mit \Macro{enlargethispage} vergrößert werden,
falls der Fuß der ersten Seite dies erlaubt, so kann das mit
\OptionValue{enlargefirstpage}{true}%
\important{\OptionValue{enlargefirstpage}{true}} erreicht werden. Es passt
dann bestenfalls etwas mehr Text auf die erste Seite.  Siehe hierzu auch die
Erklärung zur Pseudolänge \DescRef{\LabelBase.plength.firstfootvpos} auf
...
Seit\ChangedAt{v3.48}{\Class{scrlttr2}\and \Package{scrletter}} Version 3.48
ist \KOMAScript{} außerdem etwas tolerant bei der Kollisionserkennung. Sollte
die Verkleinerung oder die Vergrößerung geringer als \Length{maxdepth}
ausfallen, so unterbleibt sie automatisch. \Length{maxdepth} wird vom Paket
\Package{typearea} normalerweise auf die Hälfte von \Length{topskip}
eingestellt. \Length{topskip} wiederum entspricht in der Regel der
Grundschriftgröße und ist damit von Option
\DescRef{\LabelBase.option.fontsize} abhängig. Bei \Class{scrlttr2} gilt dies
nur, wenn nicht per Option \DescRef{\LabelBase.option.version}
Kompatibilität zu einer früheren Version erzwungen wird.%
\EndIndexGroup
...
Hilfe der \Macro{parbox}-Anweisung (siehe \cite{latex:usrguide-historic})
mehrere Boxen neben- und untereinander gesetzt werden.  Einem versierten
Anwender sollte es so möglich sein, eigene Seitenköpfe und "~füße zu
gestalten.  Natürlich kann und sollte im \PName{Inhalt} mit Hilfe von
\DescRef{\LabelBase.cmd.usekomavar} auch auf weitere Variablen zugegriffen
werden.
