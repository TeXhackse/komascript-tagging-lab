...
\KOMAProvidesFile{scrbase-de.tex}
                 [$Date: 2026-08-28 14:51:39 +0200 (Fri, 28 Aug 2026) $
...
\begin{Declaration}
  \Macro{ExpandArgsCS}\Parameter{Spezifikation}\Parameter{Anweisungsname}\Parameter{Argument\textsubscript{1}}\dots
\end{Declaration}%
Seit\ChangedAt{v3.49.3}{\Package{scrbase}} Version 2022-06-01 bietet der
\LaTeX-Kern eine Anweisung \Macro{ExpandArgs}, mit deren Hilfe man die an eine
Anweisung übergebenen Argumente entsprechend einer \PName{Spezifikation}
expandieren kann. In der L3-Programmierebene entspricht dies den Funktionen
\Macro{exp\_args:N\dots}. Während die L3-Programmierebene auch Funktionen
\Macro{exp\_args:c\dots} bereitstellt, mit deren Hilfe man dieselbe
Funktionalität auch nutzen kann, wenn die Anweisung noch nicht als Makro,
sondern nur als \PName{Anweisungsname} --~in der L3-Sprechweise als
\emph{csname} bezeichnet~-- verfügbar ist, fehlt eine Entsprechung auf
\LaTeXe-Ebene. \Macro{ExpandArgsCS} ist diese Entsprechung. Der Aufruf
\Macro{ExpandArgsCS}\Parameter{Anweisungsname}\Parameter{Spezifikation}\dots
entspricht also
\Macro{ExpandArg}\Macro{\PName{Anweisungsname}}\Parameter{Spezifikation}\dots.
Näheres zu \Macro{ExpandArgs} ist \cite{latex:usrguide} zu entnehmen. Die
Dokumentation von \Macro{exp\_args:\dots} ist in \cite{latex:interface3} zu
finden.%
\EndIndexGroup
