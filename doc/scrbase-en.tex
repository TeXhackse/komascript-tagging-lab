...
\begin{Declaration}
  \Macro{ExpandArgsCS}\Parameter{specification}\Parameter{command name}\Parameter{parameter\textsubscript{1}}\dots
\end{Declaration}
Since\ChangedAt{v3.49.3}{\Package{scrbase}} version 2022-06-01, the \LaTeX{}
kernel provides a \Macro{ExpandArgs} command that allows you to expand the
parameters passed to a command according to a \PName{specification}. In the L3
programming level, this corresponds to the \Macro{exp\_args:N\dots}
functions. While the L3 programming level also provides
\Macro{exp\_args:c\dots} functions, which allow you to use the same
functionality even if the command is not yet available as a macro but only as
\PName {command name}—referred to as \emph{csname} in L3 terminology—there is
no equivalent at the \LaTeXe{} level. \Macro{ExpandArgsCS} is this
equivalent. The call
\Macro{ExpandArgsCS}\Parameter{csname}\Parameter{specification}\dots
is therefore equivalent to
\Macro{ExpandArg}\Macro{\PName{csname}}\Parameter{specification}\dots. For
more details on \Macro{ExpandArgs}, see \cite{latex:usrguide}. The
documentation for \Macro{exp\_args:\dots} can be found in
\cite{latex:interface3}.%
\EndIndexGroup
