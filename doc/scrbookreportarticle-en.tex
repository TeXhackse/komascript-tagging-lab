...
% Copyright (c) Markus Kohm, 2001-2026
%
...
\KOMAProvidesFile{scrbookreportarticle-en.tex}
                 [$Date: 2026-08-28 18:00:05 +0200 (Fri, 28 Aug 2026) $
...
\fi
  ...
  \IndexCmd{frontmatter} and \DescRef{\LabelBase.cmd.mainmatter}%
...
\begin{Declaration}
  \Macro{frontmatter}\OParameter{numbering style}
  \Macro{frontmatter*}
  \Macro{mainmatter}\OParameter{numbering style}
  \Macro{mainmatter*}
  \Macro{backmatter}\OParameter{numbering style}
  \Macro{backmatter*}
\end{Declaration}%
The front matter includes all the material which appears before the main text
begins, including title pages, preface, and table of contents. It is initiated
with \Macro{frontmatter}\important{\Macro{frontmatter}}. In the front matter,
Roman numerals are used for the page
numbers. If\ChangedAt{v3.49}{\Class{scrbook}} a different numbering style is
to be used, this can be set using an optional argument. All specifications
that are also possible for \DescRef{\LabelBase.cmd.pagenumbering}%
\important{\DescRef{\LabelBase.cmd.pagenumbering}} from
\autoref{sec:\LabelBase.pagestyle}, \DescPageRef{\LabelBase.cmd.pagenumbering}
are permitted here. If the page numbering is to be continuous, i.\,e., not
changed by \Macro{frontmatter}, either an empty optional argument or the star
form \Macro{frontmatter*} can be used.

Chapter headings in the front matter are not numbered. However, section
headings are numbered consecutively, starting from chapter 0. This typically
does not matter, as the front matter is used only for the title pages, table
of contents%
...
The part of the book with the main text is introduced with
\Macro{mainmatter}\important{\Macro{mainmatter}}. If there is no front matter,
you can omit this command. The default page numbering in the main matter uses
Arabic numerals and (re)starts the page count at 1 at the start of the main
matter. If\ChangedAt{v3.49}{\Class{scrbook}} a different numbering style is to
be used, this can be set using an optional argument. All specifications that
are also possible for \DescRef{\LabelBase.cmd.pagenumbering}%
\important{\DescRef{\LabelBase.cmd.pagenumbering}} from
\autoref{sec:\LabelBase.pagestyle}, \DescPageRef{\LabelBase.cmd.pagenumbering}
are permitted here. If the page numbering is to be continuous, i.\,e., not
changed by \Macro{mainmatter}, either an empty optional argument or the star
form \Macro{mainmatter*} can be used.
...
numbering is not reset by default. If\ChangedAt{v3.49}{\Class{scrbook}} a
different page numbering style is
to be used, this can be set using an optional argument. All specifications
that are also possible for \DescRef{\LabelBase.cmd.pagenumbering}%
\important{\DescRef{\LabelBase.cmd.pagenumbering}} from
\autoref{sec:\LabelBase.pagestyle}, \DescPageRef{\LabelBase.cmd.pagenumbering}
are permitted here. The star variant \Macro{backmatter*} has the same
result as the normal variant \Macro{backmatter} without optional argument or
with an empty optional argument.%
%
\EndIndexGroup
...
argument will be interpreted as an \PName{option list}.  Five
options\,---\,\OptionVName{head}{running head}\important{\Option{head},
  \Option{nohead}, \Option{nonumber}, \Option{reference}, \Option{tocentry}},
\OptionVName{nohead}{simple switch}\ChangedAt{v3.49.3}{\Class{scrbook}\and
  \Class{scrreprt}\and \Class{scrartcl}}, \OptionVName{nonumber}{simple
  switch}\ChangedAt{v3.27}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}}, \OptionVName{reference}{reference title}%
\ChangedAt{v3.22}{\Class{scrbook}\and \Class{scrreprt}\and \Class{scrartcl}},
and \OptionVName{tocentry}{table of contents entry}\,---\,are supported with
this format. To use commas or equals signs within the values of \Option{head},
\Option{reference} or \Option{tocentry}, you must enclose them in braces.
...
By using the option \OptionValue{nohead}{true}%
\important{\Option{nohead}}\ChangedAt{v3.49.3}{\Class{scrbook}\and
  \Class{scrreprt}\and \Class{scrartcl}}, you can disable the generation of an
automatic running head for a heading, if such a title is used. For more
information on automatic running heads, see the description of
\DescRef{\LabelBase.cmd.chaptermark} through
\DescRef{\LabelBase.cmd.subsectionmark},
\DescPageRef{\LabelBase.cmd.chaptermark}, and the description of the page
style \DescRef{\LabelBase.pagestyle.headings} in
\autoref{sec:maincls.pagestyle}, \DescPageRef{\LabelBase.pagestyle.headings},
or in \autoref{cha:scrlayer-scrpage},
...
is, in \Class{book}, \Class{scrbook}, \Class{report} and \Class{scrreprt},
...
head\Index{running head}. \ChangedAt{v3.49.3}{\Class{scrbook}\and
  \Class{scrreprt}\and \Class{scrartcl}}The use of the asterisk variant thus
corresponds to the non-asterisk main form with the optional arguments
\OParameter{\normalfont\PValue{\OptionValue{tocentry}{\{\}},
    \OptionValue{nohead}{true}\texttt, \OptionValue{nonumber}{true}}},
provided that the extended interpretation of the optional argument is
active. See the preceding explanation regarding \DescRef{\LabelBase.cmd.part}
through \DescRef{\LabelBase.cmd.subparagraph}.

The absence of a running head often has an unwanted side effect. If, for
example, a chapter set using \Macro{chapter*} spans several pages, then the
running head of the previous chapter suddenly reappears.  {\KOMAScript} offers
a solution for this problem, described below.
...
\Class{report} and \Class{scrreprt}, not in the article classes
...
\Class{book}, \Class{scrbook}, \Class{report} and \Class{scrreprt}, not in
...
\Class{scrreprt}\IndexClass{scrreprt} classes number the section headings
