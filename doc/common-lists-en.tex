...
\IfThisCommonLabelBase{scrextend}{\iffalse}{\csname iftrue\endcsname}
  ...
  default of \FontElement{itemizelabel} itself is \Macro{normalfont}.%
  ...
  Every item is introduced with \Macro{item}.%
...
\end{lstcode}
  ...
  all other levels are followed by a dot.
  ...
  With the \KOMAScript{} classes the
  fonts\Index{font>style}\IfThisCommonLabelBase{scrlttr2}{\ChangedAt{v3.44}{\Class{scrlttr2}\and
      \Package{scrletter}}}{\ChangedAt{v3.44}{\Class{scrbook}\and
      \Class{scrreprt}\and \Class{scrartcl}}} used to format the numbers
  respective labels for the different levels can be changed using
  \DescRef{\ThisCommonLabelBase.cmd.setkomafont} and
  \DescRef{\ThisCommonLabelBase.cmd.addtokomafont} (see
  \autoref{sec:\ThisCommonLabelBase.textmarkup},
  \DescPageRef{\ThisCommonLabelBase.cmd.setkomafont}) for the elements
  \FontElement{labelenumi}\important{\FontElement{labelenumi}},
  \FontElement{labelenumii}\important{\FontElement{labelenumii}} and
  \FontElement{labelenumiii}\important{\FontElement{labelenumiii}} and
  \FontElement{labelenumiv}\important{\FontElement{labelenumiv}}. By default
  these all use the font setting for element
  \FontElement{enumeratelabel}\important{\FontElement{enumeratelabel}}. The
  default of \FontElement{enumeratelabel} itself is empty, which means no
  change of the current font.%
  ...
  \EndIndex{FontElement}{enumeratelabel} Every item is introduced with
  \Macro{item}.%
  \IfThisCommonFirstRun{\iftrue}{\csname iffalse\endcsname}
    \begin{Example}
      \phantomsection\xmpllabel{env.enumerate}%
      Let's shorten the previous example and replace the
      \DescRef{\ThisCommonLabelBase.env.itemize} environment by an
