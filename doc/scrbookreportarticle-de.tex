...
\fi
  ...
  \IndexCmd{frontmatter} und \DescRef{\LabelBase.cmd.mainmatter}%
...
\begin{Declaration}
  \Macro{frontmatter}\OParameter{Nummerierungsstil}
  \Macro{frontmatter*}
  \Macro{mainmatter}\OParameter{Nummerierungsstil}
  \Macro{mainmatter*}
  \Macro{backmatter}\OParameter{Nummerierungsstil}
  \Macro{backmatter*}
\end{Declaration}%
Mit \Macro{frontmatter}\important{\Macro{frontmatter}} wird der Vorspann
eingeleitet. Dadurch werden die nummerierten Seiten mit römischen Seitenzahlen
versehen. Soll\ChangedAt{v3.49}{\Class{scrbook}} ein anderer
\PName{Nummerierungsstil} verwendet werden, so kann dieser per optionalem
Argument eingestellt werden. Erlaubt sind hier alle Angaben, die auch für
\DescRef{\LabelBase.cmd.pagenumbering}%
\important{\DescRef{\LabelBase.cmd.pagenumbering}} aus
\autoref{sec:\LabelBase.pagestyle}, \DescPageRef{\LabelBase.cmd.pagenumbering}
möglich sind. Soll die Seitennummerierung durchgehend erfolgen, sich also
durch \Macro{frontmatter} nicht ändern, so kann wahlweise ein leeres
optionales Argument oder die Sternform \Macro{frontmatter*} verwendet werden.

Kapitelüberschriften sind im Vorspann nicht
...
Kapitelnummern gegeben ist! Die Verwendung von \DescRef{\LabelBase.cmd.addsec}
und \DescRef{\LabelBase.cmd.section*} (siehe
\autoref{sec:\LabelBase.structure}, \DescPageRef{\LabelBase.cmd.section*} und
\DescPageRef{\LabelBase.cmd.addsec}) sind aus Sicht des Autors im Vorspann
...
Seitenzählung beginnt im Hauptteil neu mit der
1. Soll\ChangedAt{v3.49}{\Class{scrbook}} ein anderer
\PName{Nummerierungsstil} verwendet werden, so kann dieser per optionalem
Argument eingestellt werden. Erlaubt sind hier alle Angaben, die auch für
\DescRef{\LabelBase.cmd.pagenumbering}%
\important{\DescRef{\LabelBase.cmd.pagenumbering}} aus
\autoref{sec:\LabelBase.pagestyle}, \DescPageRef{\LabelBase.cmd.pagenumbering}
möglich sind. Soll die Seitennummerierung durchgehend erfolgen, sich also
durch \Macro{mainmatter} nicht ändern, so kann wahlweise ein leeres
optionales Argument oder die Sternform \Macro{mainmatter*} verwendet werden.
...
jedoch nicht voreingestellt. Soll\ChangedAt{v3.49}{\Class{scrbook}} für die
Seiten dennoch ein anderer \PName{Nummerierungsstil} verwendet werden, so kann
dieser per optionalem Argument eingestellt werden. Erlaubt sind hier alle
Angaben, die auch für \DescRef{\LabelBase.cmd.pagenumbering}%
\important{\DescRef{\LabelBase.cmd.pagenumbering}} aus
\autoref{sec:\LabelBase.pagestyle}, \DescPageRef{\LabelBase.cmd.pagenumbering}
möglich sind. Die Sternform \Macro{backmatter*} führt zu demselben Ergebnis
wie die Normalform \Macro{backmatter} ohne oder mit leerem optionalem
Argument.%
%
\EndIndexGroup
...
werden die fünf Optionen
\OptionVName{head}{Kolumnentitel}\important{\Option{head}, \Option{nohead},
  \Option{nonumber}, \Option{reference}, \Option{tocentry}},
\OptionVName{nohead}{Ein-Aus-Wert}\ChangedAt{v3.49.3}{\Class{scrbook}\and
  \Class{scrreprt}\and \Class{scrartcl}},
...
\OptionVName{tocentry}{Inhaltsverzeichniseintrag}, akzeptiert. Um ein
Gleichheitszeichen oder ein Komma in einem der Werte von \Option{head},
\Option{reference} oder \Option{tocentry} unterzubringen, muss dieses in
zusätzliche geschweifte Klammern gesetzt werden.
...
Durch Verwendung von Option \OptionValue{nohead}{true}%
\important{\Option{nohead}}\ChangedAt{v3.49.3}{\Class{scrbook}\and
  \Class{scrreprt}\and \Class{scrartcl}} kann man die Erzeugung eines
automatische Kolumnentitel für die eine Überschrift deaktivieren, falls diese
verwendet werden. Näheres zu automatischen Kolumnentiteln finden Sie bei der
Beschreibung zu \DescRef{\LabelBase.cmd.chaptermark} bis
\DescRef{\LabelBase.cmd.subsectionmark},
\DescPageRef{\LabelBase.cmd.chaptermark}, bei der Beschreibung des Seitenstils
\DescRef{\LabelBase.pagestyle.headings} in \autoref{sec:maincls.pagestyle},
\DescPageRef{\LabelBase.pagestyle.headings} oder in
\autoref{cha:scrlayer-scrpage}, \DescPageRef{scrlayer-scrpage.cmd.automark}.
...
vorgenommen. \ChangedAt{v3.49.3}{\Class{scrbook}\and \Class{scrreprt}\and
  \Class{scrartcl}}Die Verwendung der Sternvariante entspricht damit der
sternlosen Hauptform mit dem optionalen Argument
\OParameter{\normalfont\PValue{\OptionValue{tocentry}{\{\}},
    \OptionValue{nohead}{true}, \OptionValue{nonumber}{true}}}, soweit die
erweiterte Interpretation des optionalen Arguments aktiv ist. Siehe dazu die
vorausgehende Erklärung zu \DescRef{\LabelBase.cmd.part} bis
\DescRef{\LabelBase.cmd.subparagraph}.

Der Verzicht auf den Kolumnentitel hat übrigens einen oftmals unerwünschten
Effekt. Geht beispielsweise ein mit \Macro{chapter*} gesetztes Kapitel über
mehrere Seiten, so taucht plötzlich der Kolumnentitel des letzten Kapitels
wieder auf.  {\KOMAScript} bietet dafür aber eine Lösung, die im Anschluss
beschrieben wird. \OnlyAt{\Class{scrbook}\and
  \Class{scrreprt}}\Macro{chapter*} existiert selbstverständlich nur bei Buch-
und Berichtklassen, also bei \Class{book}, \Class{scrbook}, \Class{report} und
\Class{scrreprt}, nicht jedoch bei den Artikelklassen \Class{article} und
\Class{scrartcl}.
...
\end{lstcode}
  ...
  \cite{latex:usrguide-historic} zu entnehmen.
\end{Example}
