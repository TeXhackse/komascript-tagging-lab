...
the other using the \Macro{parbox} command (see \cite{latex:usrguide-historic}).
...
\begin{Declaration}
  \OptionVName{shrinkfirstpage}{simple switch}
...
\end{Explain}
If\ChangedAt{v3.48}{\Class{scrlttr2}\and \Package{scrletter}} the text area
should not automatically become smaller to avoid collision of the text and the
footer, you can use option \OptionValue{shrinkfirstpage}{false}%
\important{\OptionValue{shrinkfirstpage}{false}}.  If the text area should
become larger, assuming the footer on the first page permits this, you can use
\OptionValue{enlargefirstpage}{true}%
\important{\OptionValue{enlargefirstpage}{true}}. At best, a little more text
will then fit on the first page. See also the description of the
\PLength{firstfootvpos} pseudo-length on
\DescPageRef{\LabelBase.plength.firstfootvpos}.  This options take the
standard values for simple switches, as listed in
\autoref{tab:truefalseswitch}, \autopageref{tab:truefalseswitch}.

Since\ChangedAt{v3.48}{\Class{scrlttr2}\and \Package{scrletter}} version 3.48,
\KOMAScript{} has also been somewhat tolerant in collision detection. If the
reduction or enlargement is less than \Length{maxdepth}, it is automatically
omitted. \Length{maxdepth} is normally set by the \Package{typearea} package
to half of \Length{topskip}. \Length{topskip}, in turn, usually corresponds to
the base font size and is therefore dependent on the option
\DescRef{\LabelBase.option.fontsize}. For \Class{scrlttr2}, this only applies
if compatibility with an earlier version is not enforced by the option
...
\cite{latex:usrguide-historic}). More advanced users should be able to create
their own headers and footers. Of course, you can and should also make use of
additional variables using \DescRef{\LabelBase.cmd.usekomavar}.
